...
# GPT2 non ha un token di padding per default; lo impostiamo uguale all'eos_token
...
# Costruisci un modello custom di classificazione basato su TFGPT2Model
def build_gpt2_classifier(model_name, num_labels):
    # Carica il modello base GPT2
    ...
    # Aggiungi dropout e testa di classificazione
...
# Costruzione del modello distilgpt2 con testa di classificazione
...
def traning_loop(model, train_dataset):
    ...
    for batch in tqdm(train_dataset, desc="Train"):
        ...
        with tf.GradientTape() as tape:
            logits = model(inputs, training=True)  # (batch, num_classes)
